...
class SocialVideoDownloader:
    ...
    def download_single_video(self, url: str, video: bool = True, audio: bool = True, subtitles: bool = True) -> Dict:
        ...
        try:
                ...
                with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                    ...
                    if subtitles:
                        for lang_code in ['vie-VN', 'eng-US']:
                            subtitle_file = os.path.join(video_folder, f"video.{lang_code}.vtt")
                            if os.path.exists(subtitle_file):
                                new_subtitle_file = os.path.join(video_folder, f"sub-{lang_code}.vtt")
            ...
            if subtitles and not result['paths']['subtitles']:
                ...
                with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                    ydl.download([url])
                    for lang_code in ['vie-VN', 'eng-US']:
                        subtitle_file = os.path.join(video_folder, f"sub.{lang_code}.vtt")
                        if os.path.exists(subtitle_file):
                            new_subtitle_file = os.path.join(video_folder, f"sub-{lang_code}.vtt")
                            os.rename(subtitle_file, new_subtitle_file)
                            result['paths']['subtitles'][lang_code] = new_subtitle_file
                ...
                if not video and 'actual_video_path' in locals() and os.path.exists(actual_video_path):
                    os.remove(actual_video_path)

                if not audio and 'actual_audio_path' in locals() and os.path.exists(actual_audio_path):
    ...
    def download_playlist_or_channel(self, url: str, download_options: Optional[Dict] = None) -> Dict:
    ...
    def download_from_urls(self, urls: Union[str, List[str]], download_options: Optional[Dict] = None) -> Dict:
        ...
        for url in urls:
            ...
            if self.is_playlist_or_channel(url):
                ...
                playlist_result = self.download_playlist_or_channel(url, download_options)
                results['downloads'].append(playlist_result)
                results['total_videos'] += playlist_result['total_videos']
                results['successful_downloads'] += playlist_result['successful_downloads']
